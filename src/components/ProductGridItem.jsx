  ...
  return (
    <div ref={ref} className={`${productStyle.product_item} second_level_blur rounded-md p-5 relative overflow-hidden`}>
      <Link href={`/products/${product.slug}`}>
        <a>
          <div className="product_img text-center w-full block mb-7">
            <Image src={product.images[0].src} width={260} height={260} type="intrinsic" alt={product.name} className="m-auto" />
          </div>

          <div className="product_info">
            <h4 className="product_title text-xl leading-6 mb-3">{product.name}</h4>
            {product.regular_price && (<p className="product_price line-through text-sm">${ product.regular_price = product.sale_price ? product.regular_price : '' }.00</p>)}
            <p className="product_price">${product.sale_price ? product.sale_price : product.price}.00</p>
          </div>
          <div
